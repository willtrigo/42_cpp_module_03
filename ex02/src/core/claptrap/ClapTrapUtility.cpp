...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/20 09:30:03 by dande-je          #+#    #+#             */
/*   Updated: 2025/03/20 09:31:12 by dande-je         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ClapTrap::takeDamage(unsigned int amount) {
  ...
  if (!this->m_hitPoints) {
    this->m_strColor = RED;
    log("takeDamage: " + this->m_nameClass + " " + this->m_name + " is dead!");
  }
}
...
void ClapTrap::beRepaired(unsigned int amount) {
  ...
  if (!this->m_hitPoints) {
    this->m_strColor = RED;
    return log("beRepaired: " + this->m_nameClass + " " + this->m_name + " is dead!");
  }
  if (!this->m_energyPoints) {
    this->m_strColor = BLUE;
    return log("beRepaired: " + this->m_nameClass + " " + this->m_name + " has no energy!");
  }
  std::ostringstream amountStr;
